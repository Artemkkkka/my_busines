...
from src.meetings.crud import update_meeting
...
async def test_update_meeting_updates_fields_and_sets_canceled_if_ends_at_in_payload(session: AsyncSession):
    ...
    updated = await update_meeting(meeting_id=m.id, payload=payload, session=session)
    ...
    with pytest.raises(HTTPException) as exc:
        await update_meeting(meeting_id=m.id + 999, payload=MeetingUpdate(title="x"), session=session)
