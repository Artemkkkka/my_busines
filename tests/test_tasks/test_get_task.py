...
from src.tasks.crud import create_task, get_all_tasks
...
async def test_get_all_tasks_filters_by_team(session: AsyncSession):
    ...
    await create_task(session, team_id=t1.id, author_id=a1.id,
                      name="T1-1", description="", deadline_at=datetime(2025, 1, 2, 9, 0, 0))
    await create_task(session, team_id=t1.id, author_id=a2.id,
                      name="T1-2", description="", deadline_at=datetime(2025, 1, 3, 9, 0, 0))
    await create_task(session, team_id=t2.id, author_id=b1.id,
                      name="T2-1", description="", deadline_at=datetime(2025, 1, 4, 9, 0, 0))

    tasks_team1 = await get_all_tasks(session, team_id=t1.id)
