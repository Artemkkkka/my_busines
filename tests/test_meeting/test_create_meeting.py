...
async def test_create_meeting_success(session: AsyncSession):
    ...
    created = await MeetingCRUD.create_meeting(user=manager, payload=payload, session=session)
...
async def test_create_meeting_conflict_overlap_raises_409(session: AsyncSession):
    ...
    await MeetingCRUD.create_meeting(user=manager, payload=first, session=session)
...
async def test_back_to_back_meetings_are_allowed(session: AsyncSession):
    ...
    await MeetingCRUD.create_meeting(user=manager, payload=p1, session=session)
    ...
    created2 = await MeetingCRUD.create_meeting(user=manager, payload=p2, session=session)
